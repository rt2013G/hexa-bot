...
async def remove_seller(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("L'utente non è più un venditore!", 
                                    reply_markup=ReplyKeyboardRemove())
    await context.bot.send_message(user.id, 
...
async def reject_seller(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_text("L'utente è stato notificato del rifiuto.", 
                                    reply_markup=ReplyKeyboardRemove())
    await context.bot.send_message(user.id, 
...
async def make_scammer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_role(user.id, "scammer"):
        await update.message.reply_text("Utente già nella lista scammer!", 
                                        reply_markup=ReplyKeyboardRemove())
        return
    
    make_role(user.id, "scammer")
    await update.message.reply_text("Utente inserito nella lista scammer!", 
                                    reply_markup=ReplyKeyboardRemove())
...
async def remove_scammer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_role(user.id, "scammer"):
        await update.message.reply_text("L'utente non è nella lista scammer!", 
                                        reply_markup=ReplyKeyboardRemove())
        return
    
    remove_role(user.id, "scammer")
    await update.message.reply_text("L'utente non è più nella lista scammer!", 
                                    reply_markup=ReplyKeyboardRemove())
    await context.bot.send_message(user.id, 
...
async def announce(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    users = get_users()
    for user in users:
        await context.bot.send_message(user.id, message, reply_markup=ReplyKeyboardRemove())
...
async def get_id_by_username(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = get_user_from_message_command(update.message.text, "/getid")
    if user is None:
        await update.message.reply_text("Utente non trovato!", 
                                        reply_markup=ReplyKeyboardRemove())
    await update.message.reply_text(f"id di @{user.username}: {user.id}", 
                                    reply_markup=ReplyKeyboardRemove())      


async def get_username_by_id(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if user.username is None:
        await update.message.reply_text("L'utente non ha uno username!", 
                                        reply_markup=ReplyKeyboardRemove())
        return
    await update.message.reply_text(f"Username di {user.id}: @{user.username}", 
                                    reply_markup=ReplyKeyboardRemove())
...
async def make_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_role(user.id, "admin"):
        await update.message.reply_text("Utente già admin!", 
                                        reply_markup=ReplyKeyboardRemove())
        return
    
    make_role(user.id, "admin")
    await update.message.reply_text("Utente aggiunto come admin!", 
                                    reply_markup=ReplyKeyboardRemove())
...
async def remove_admin(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_role(user.id, "admin"):
        await update.message.reply_text("L'utente non è admin!", 
                                        reply_markup=ReplyKeyboardRemove())
        return
